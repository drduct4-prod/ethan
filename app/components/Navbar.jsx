...
const COUNTRY_CONFIG = {
  us: { img: "/us_flag.png", flag: "🇺🇸", label: "US" },
  ca: { img: "/ca_flag.png", flag: "🇨🇦", label: "CA" },
};
...
const SERVICE_SLUGS = [
  ...
  { slug: "heat-pump-cleaning",      href: "/services/heat-pump-cleaning",      icon: Thermometer },
  { slug: "dryer-vent-cleaning",     href: "/services/dryer-vent-cleaning",     icon: Shirt },
  { slug: "furnace-blower-cleaning", href: "/services/furnace-blower-cleaning", icon: Fan },
...
export default function Navbar() {
  ...
  useEffect(() => { setSelectedCountry(pathCountry); }, [pathCountry]);

  const country = COUNTRY_CONFIG[selectedCountry];

  const handleCountryChange = useCallback((code) => {
    if (code === selectedCountry) { setCountryDropdownOpen(false); return; }
  ...
  const withCountry = useCallback((href) => {
  ...
  const openDropdown = () => {
    ...
    gsap.to(dropdownWrapRef.current,  { height: "auto", duration: 0.38, ease: "power3.out" });
    gsap.to(dropdownPanelRef.current, { y: "0%",        duration: 0.38, ease: "power3.out" });
    if (dropdownCardsRef.current) {
      gsap.fromTo(
        dropdownCardsRef.current.querySelectorAll(".svc-card"),
        { opacity: 0, y: 12 },
        { opacity: 1, y: 0, duration: 0.24, stagger: 0.04, ease: "power2.out", delay: 0.1 }
      );
    }
  };
  ...
  const closeDropdown = () => {
    ...
    leaveTimerRef.current = setTimeout(() => {
      ...
      gsap.killTweensOf(dropdownWrapRef.current);
      gsap.killTweensOf(dropdownPanelRef.current);
      gsap.to(dropdownPanelRef.current, { y: "-100%", duration: 0.26, ease: "power2.in" });
      gsap.to(dropdownWrapRef.current,  { height: 0,   duration: 0.28, ease: "power2.in", delay: 0.04 });
    }, 90);
  };
    ...
    <>
      ...
      >
        ...
        <div ref={countryDropdownRef} className="relative">
          ...
          >
            <Image src={country.img} alt={`${country.label} flag`} width={20} height={15} className="rounded-xs" />
            <span className="text-[12px] font-semibold text-gray-700 uppercase tracking-wide">{country.label}</span>
            <ChevronDown className={`h-3 w-3 text-gray-500 transition-transform duration-200 ${countryDropdownOpen ? "rotate-180" : ""}`} />
          </button>
          {countryDropdownOpen && (
            <div className="absolute left-0 top-full z-50 mt-1.5 min-w-27.5 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-lg" role="listbox">
              {Object.entries(COUNTRY_CONFIG).map(([code, cfg]) => (
                <button key={code} role="option" aria-selected={code === selectedCountry}
                  onClick={() => handleCountryChange(code)}
                  className={`flex w-full items-center gap-2 px-3 py-2 text-[13px] font-medium transition-colors hover:bg-gray-50 ${code === selectedCountry ? "text-gray-900 bg-gray-50" : "text-gray-600"}`}
                >
                  <Image src={cfg.img} alt={`${cfg.label} flag`} width={18} height={13} className="rounded-xs" />
                  {cfg.label}
                  {code === selectedCountry && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-[#5E7AC4]" />}
                </button>
              ))}
            </div>
          )}
        ...
        <div ref={langDropdownRef} className="relative">
          ...
          >
            ...
            <span className="text-[12px] font-semibold text-gray-700 uppercase tracking-wide">{lang}</span>
            <ChevronDown className={`h-3 w-3 text-gray-500 transition-transform duration-200 ${langDropdownOpen ? "rotate-180" : ""}`} />
          </button>
          {langDropdownOpen && (
            <div className="absolute left-0 top-full z-50 mt-1.5 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-lg" role="listbox">
              {LANG_OPTIONS.map(({ code, label }) => (
                ...
                >
                  {label}
                  {code === lang && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-[#5E7AC4]" />}
                </button>
              ))}
            </div>
          )}
        ...
        <span className="text-[0.6rem] lg:text-[0.9rem]">
          <strong className="font-bold">{t.nav.promoStrong}</strong> {t.nav.promoRest}
        </span>
      </div>

      {/* ── Main navbar ── */}
      <header className={`sticky top-0 z-40 w-full border-b border-white/10 bg-black ${urbanist.className}`}>
        <nav className="mx-auto flex h-17 max-w-350 items-center justify-between px-5 lg:px-10">
          ...
          <Link href={withCountry("/")} className="flex shrink-0 items-center gap-2">
            <Image src="/black_logo.png" alt="Ethan Duct Cleaning" width={260} height={40} loading="eager" priority />
          </Link>
          ...
          <div className="flex items-center gap-2 lg:hidden">
            <span className="text-lg leading-none select-none" role="img" aria-label={`${country.label} flag`}>{country.flag}</span>
            <button className="flex h-9 w-9 items-center justify-center rounded-md text-white hover:bg-white/10 transition-colors"
        ...
        >
          <div ref={dropdownPanelRef} className="w-full border-t border-gray-100 bg-white shadow-lg">
            <div className="mx-auto max-w-7xl px-5 py-8 lg:px-10">
              <div ref={dropdownCardsRef} className="grid grid-cols-3 gap-x-10 gap-y-6">
                {SERVICE_SLUGS.map((svc) => {
                  const Icon = svc.icon;
                  const info = t.servicesSection.list[svc.slug];
                  return (
                    <Link key={svc.slug} href={withCountry(svc.href)}
                    ...
                    >
                      <div className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-gray-200 bg-white transition-colors duration-150 group-hover:border-[#5E7AC4]/40 group-hover:bg-[#5E7AC4]/5">
                        <Icon className="h-4.5 w-4.5 text-gray-600 transition-colors duration-150 group-hover:text-[#5E7AC4]" strokeWidth={1.5} />
                      </div>
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-[13px] leading-snug text-gray-500">{info.description}</p>
                      </div>
                    </Link>
                  );
                })}
      ...
      {/* ── Mobile overlay ── */}
      <div ref={mobileOverlayRef} className="fixed inset-0 z-50 bg-black/50 backdrop-blur-[2px] lg:hidden"
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom:"1px solid rgba(255,255,255,0.08)" }}>
          <Image src="/black_logo.png" alt="Logo" width={120} height={28} className="brightness-200" />
          ...
          <div className="flex items-center gap-1">
            {Object.entries(COUNTRY_CONFIG).map(([code, cfg]) => (
              <button key={code} onClick={() => handleCountryChange(code)}
                className={`flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[12px] font-medium uppercase tracking-widest transition-colors ${code === selectedCountry ? "bg-white/10 text-white" : "text-white/40 hover:bg-white/5 hover:text-white/70"}`}
        ...
        <div className="flex-1 overflow-y-auto px-3 py-3">
          ...
          <div>
            ...
            >
              {t.nav.services}
              <ChevronDown className={`h-4 w-4 text-white/40 transition-transform duration-300 ${mobileServicesOpen ? "rotate-180 text-[#5E7AC4]" : ""}`} />
            </button>
            <div ref={mobileServicesRef} className="overflow-hidden" style={{ height:0, opacity:0 }}>
                ...
                {SERVICE_SLUGS.map((svc) => {
                  ...
                  return (
                    <Link key={svc.slug} href={withCountry(svc.href)}
                      ...
                      onClick={() => setMobileOpen(false)}
                    >
                      <Icon className="h-4 w-4 shrink-0 text-[#5E7AC4]" strokeWidth={1.5} />
                      {info.title}
                    </Link>
                  );
                })}
          ...
          {NAV_LINKS.filter((l) => !l.isDropdown && !l.isAreasDropdown).map((link) =>
            link.isAnchor ? (
              ...
              <Link key={link.href} href={withCountry(link.href)}
                className={`flex items-center justify-between rounded-xl px-4 py-3 text-[15px] font-medium transition-colors ${isActive(link.href) ? "bg-white/5 text-[#5E7AC4]" : "text-white/80 hover:bg-white/5 hover:text-white"}`}
                onClick={() => setMobileOpen(false)}
              >
                {t.nav[link.key]}
              </Link>
        ...
        <div className="p-4 space-y-2" style={{ borderTop:"1px solid rgba(255,255,255,0.08)" }}>
          <Link href={withCountry("/contact-us")}
